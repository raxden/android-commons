...
fun <T, R, E> Flow<ResultData<T, E>>.then(function: suspend (value: T) -> R): Flow<ResultData<R, E>> =
    map { result -> result.coThen { function(it) } }

fun <T, R, E> Flow<ResultData<T, E>>.thenFailure(function: (value: E) -> R): Flow<ResultData<T, R>> =
